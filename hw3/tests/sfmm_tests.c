...
Test(sf_memsuite_student, test_malloc, .init = sf_mem_init, .fini = sf_mem_fini) {
...
//############################################
Test(sf_memsuite_student, realloc_greater_than_4_pages, .init = sf_mem_init, .fini=sf_mem_fini){
    void * x = sf_malloc(10);
    void * y = sf_realloc(x, 16384);

    cr_assert_null(y, "y not set to NULL!");
    cr_assert(errno == ENOMEM, "errno not set to ENOMEM!");
}

Test(sf_memsuite_student, realloc_then_free, .init = sf_mem_init, .fini=sf_mem_fini){
    void * x= sf_malloc(50);
    void * y= sf_realloc(x, 100);

    sf_free(y); //both x and y should be freeds

    free_list *fl_y = &seg_free_list[find_list_index_from_size(100)];
    cr_assert_null(fl_y->head->next, "More than one item in list 1!");
    cr_assert_null(fl_y->head->prev, "Head has a prev pointer!");

}

Test(sf_memsuite_student, realloc_same_amount, .init=sf_mem_init, .fini=sf_mem_fini){
    void * x = sf_malloc(10);
    void * y = sf_realloc(x, 10);

    sf_header * x_head = (sf_header *)x;
    sf_header * y_head = (sf_header *)y;
    cr_assert(x_head->block_size<<4 == y_head->block_size, "x and y are not equal!");
}

Test(sf_memsuite_student, realloc_larger_block_then_small_splinter, .init=sf_mem_init, .fini=sf_mem_fini){
    void * x = sf_malloc(100);
    void * y = sf_realloc(x, 567);
    void * z = sf_realloc(y, 555);

    sf_header * z_head = z-8;
    free_list *fl_x = &seg_free_list[find_list_index_from_size(100)];
    cr_assert(z_head->block_size<<4 == 592, "z doesn't have a block size of 592!");
    cr_assert_null(fl_x->head->next, "More than 1 item in list 1!");
}

Test(sf_memsuite_student, realloc_larger_block_then_small_no_splinter, .init=sf_mem_init, .fini=sf_mem_fini){
    void * x = sf_malloc(100);
    void * y = sf_realloc(x, 567);
    void * z = sf_realloc(y, 544);

    sf_header * z_head = z-8;

    free_list *fl_x = &seg_free_list[find_list_index_from_size(100)];
    cr_assert(z_head->block_size<<4 == 560, "block size isn't 560!");
    cr_assert_null(fl_x->head->next, "More than 1 item in list 1!");
}
